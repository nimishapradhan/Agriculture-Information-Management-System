...
<?php
    ...
    $mysql_query = "SELECT * FROM admin_details WHERE password = '$currentPass' and id = '$currentUserID'";
    $established_connection = $mysql_connection -> query($mysql_query);
    if ($established_connection -> num_rows > 0) {
        if($password === $confirmpassword) {
            if (strlen($password) >= 6) {
                $mysql_query = "UPDATE admin_details set password = '$password' where id = '$currentUserID'";
